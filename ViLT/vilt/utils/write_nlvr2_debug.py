...
def process_blob(root, iden, row):
    texts = [r["sentence"] for r in row]
    labels = [r["label"] for r in row]
    ...
    if iden.startswith("train"):
        directory = row[0]["directory"]
        path = f"{root}/raw_img/images/train/{directory}/{iden}"
    else:
        path = f"{root}/raw_img/{split}/{iden}"
...
if __name__ == '__main__':
    make_blob_arrow('/blob/v-jinx/data/nlvr2', '/blob/v-jinx/data/VilT_dataset_debug')